...
impl Slab {
    ...
    pub fn insert(&mut self, key: u128, price: u64, qty: u64, owner: Pubkey) -> Result<()> {
        ...
        if let Some(old) = self.head {
            self.nodes[old as usize].prev = Some(idx);
        }
        self.head = Some(idx);
        Ok(())
    }
    ...
    pub fn reduce_order(&mut self, idx: u32, qty: u64) -> Result<()> {
        let (prev, next, remaining) = {
            ...
            let rem = n.qty.saturating_sub(qty);
            (n.prev, n.next, rem)
        };
        if remaining == 0 {
            if let Some(p) = prev {
    ...
    pub fn find_best(&self) -> Option<u32> {
        ...
        for node in &self.nodes {
            if node.qty == 0 { continue; }
            best_price = Some(match best_price {
                None => node.price,
                Some(bp) => if self.side == Side::Bid { bp.max(node.price) } else { bp.min(node.price) },
            });
        }
